...
from monai.data import MetaTensor   # JH added
#from collections.abc import Sequence # JH added
...
# Uncrop image:
# NB: output without channel dimension!!
class Unpadd(MapTransform):
    def __init__(
        self,
        keys: KeysCollection,
        ref_image: str,
    ...
    def __call__(self, data):
        ...
        # Get the the meta dict from whatever is fed into function
        meta_dict = (
        ...
        for idx, key in enumerate(self.keys):   # if "label" and "image" etc. are specified as keys
            result = d[key]
            current_size = result.shape[1:] if self.has_channel else result.shape
            spatial_shape = meta_dict.get("spatial_shape", current_size)
            spatial_size = spatial_shape[-len(current_size) :]  # get rid of channels/batch etc.

            # Undo the padding
            if np.any(np.not_equal(current_size, spatial_size)):
                recropper = SpatialCrop(roi_start=[0,0,0], roi_end=spatial_size)    # NB: start at origin
                result = recropper(result)
