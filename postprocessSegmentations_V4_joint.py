...
def plot_profiles_segmentation(normed_profiles, stat, metric, log_id, out_dir):
    ...
    preferred_order = ["CT", "LPD-seq", "LPD-jnt"]
...
def plot_critical_region_contours(
    ...
    for case in contour_cases:
        ...
        for variant in ["LPD-seq", "LPD-jnt"]:
            variant_folder = seg_variant_dirs[variant]
            candidate_dirs = [
            ...
            for variant in seg_variants:
                ...
                if variant_dir is None:
                    raise FileNotFoundError(
                        f"No segmentation directory found for '{variant}' in {candidate_dirs}"
                    )
...
def main(metric="dice", reload=True):
    ...
    root_seg = "/media/Store-SSD/Stembank/pine-LPDseg/"
    root_out = "/home/aime/monai/Postprocessing/LPDseg"
    ...
    # Only segmentation-pipeline comparison against CT baseline
    ordered_variants = ["GT", "CT", "LPD-seq", "LPD-jnt"]
    short_names = ["CT", "LPD seg. seq.", "LPD seg. joint"]
    short_groups = ["baseline", "segmentation", "segmentation"]
    seg_variants = ["LPD-seq", "LPD-jnt"]
    seg_variant_dirs = {
        "LPD-seq": "sequential",
        "LPD-jnt": "joint",
    }
    ...
    latex_table_slices = summary_slices_df.to_latex(multicolumn=True,
                                    multirow=True,
                                    caption=f"Mean {metric.capitalize()} scores for knot segmentation comparing CT baseline with sequential and joint segmentation pipelines on validation and test datasets.",
                                    label=f"tab:segpipeline-slicewise-{metric}",
                                    float_format="%.3g",
                                    bold_rows=True)
    
    latex_table_volumes = summary_volumes_df.to_latex(multicolumn=True,
                                    multirow=True,
                                    caption=f"Mean {metric.capitalize()} volume scores for knot segmentation comparing CT baseline with sequential and joint segmentation pipelines on validation and test datasets.",
                                    label=f"tab:segpipeline-volume-{metric}",
                                    float_format="%.3g",
                                    bold_rows=True)
