...
def plot_aggregate_mean_plus_sd(aggregate_rows, aggregate_scope, dataset, log_id, out_file):
    rows = [row for row in aggregate_rows if (
        row["aggregate_scope"] == aggregate_scope
        and row["dataset"] == dataset
        and row["log_id"] == log_id
    ...
    for variant in ["CT"] + SEG_VARIANTS:
        variant_rows = [
            row for row in rows
            if row["variant"] == variant and row["mean_dice"] != "" and row["std_dice"] != ""
        ]
        if not variant_rows:
            continue
        variant_rows = sorted(variant_rows, key=lambda row: int(row["point_idx"]))
        x = np.asarray([float(row["relative_idx"]) for row in variant_rows])
        mean = np.asarray([float(row["mean_dice"]) for row in variant_rows])
        std = np.asarray([float(row["std_dice"]) for row in variant_rows])
        style = PLOT_STYLE.get(variant, {"color": "black", "linestyle": "solid"})
        color = style["color"]
        ax.plot(
            x,
...
def plot_aggregate_profiles(aggregate_rows, root_out):
    ...
    for aggregate_scope, dataset, log_id in groups:
        ...
        out_file = os.path.join(
            ...
            group_dir,
            dataset,
            f"{log_id}_aggregate_mean_plus_SD_dice_profile.pdf",
        )
        plot_aggregate_mean_plus_sd(aggregate_rows, aggregate_scope, dataset, log_id, out_file)
